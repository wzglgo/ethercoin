...
const PROGPOW_START: u64 = 5_369_709;
...
impl EthashParams {
    ...
    pub fn dump_etg_info(&self) {
        // log all the necessary info for ETE network

        info!(target: "ete", "ETE dev accounts:[");
        for (idx, addr) in self.etg_hardfork_dev_accounts.iter().enumerate() {
            info!(target: "ete", "  {:?}: {:?}", idx + 1, addr);
        }
        info!(target: "ete", "]");
    }
}
...
fn verify_block_unordered(pow: &Arc<EthashManager>, header: &Header) -> Result<(), Error> {
	...
	if mix != seal.mix_hash {
		warn!(target: "rpc", "aaaa");
		warn!(target: "rpc", mix);
		warn!(target: "rpc", seal.mix_hash);
		warn!(target: "rpc", "bbbb");
